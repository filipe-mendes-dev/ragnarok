...
export default async function ConversationPage({ params }: { params: Promise<{ conversationId: string }> }) {
    ...
    if (!current) notFound();
    return <ChatView key={conversationId} conversationId={conversationId} conversations={await service.listConversations(user.id)} messages={current.messages} title={current.title} />;
}
